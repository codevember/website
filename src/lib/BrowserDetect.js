...
window.Device = (function() {
  ...
  Device.prototype._hasClass = function(class_name) {
    var regex;
    regex = new RegExp("\\b("+class_name+")\\b", 'i');
  ...
  Device.prototype._removeClass = function(class_name) {
    if (this._hasClass(class_name)) {
      var regex;
      regex = new RegExp("\\b("+class_name+")\\b", 'i');
      return this._doc_element.className = this._doc_element.className.replace(regex, "");
    }
  };
  ...
  Device.prototype.screenOrientation = function() {
    return (window.screen.availHeight / window.screen.availWidth) > 1 ? 'portrait' : 'landscape'
  }

  Device.prototype._handleScreenOrientation = function() {
    if (this.screenOrientation() === 'portrait') {
      this._removeClass('screenlandscape')
      this._addClass('screenportrait')
    } else {
      this._removeClass('screenportrait')
      this._addClass('screenlandscape')
    }
  }
...
BrowserDetectSingleton.get().bindEvents();

module.exports = BrowserDetectSingleton.get();
